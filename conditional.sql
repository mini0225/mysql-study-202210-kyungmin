/*<<<<<<<<<<<<<<<<<<제어문>>>>>>>>>>>>>>>>>>*/
/*	자바와 다른 단순 삼항연산자
	1. if(조건, 조건이 참일 경우의 결과, 조건이 거짓일 경우의 결과)
*/
...
select
    ...
    name as 이름,
	age as 나이,
	if(age between 20 and 29, '20대', '20대가 아님') as age_flag, /*column 명 이대로 가버림..->알리아스(Alias) : 별칭, 별명*/
	1 as num
from
	student_study_mst;
...
select 
    ...
    ifnull(age, 0) as `ifnull` /*null 이면 0, 아니면 자기나이 나옴*/
from
	student_study_mst;
...
select distinct /*select 결과에서 중복을 제거*/
